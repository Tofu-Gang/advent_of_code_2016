...
def puzzle_1() -> None:
    ...
    with open(join(getcwd(), SELF_DIR_NAME, INPUT_TXT_NAME), "r") as f:
        ...
        decompressed_output = ""
        marker_pattern = compile("\(\d+x\d+\)")
        while True:
            try:
                marker = marker_pattern.search(compressed_input).group()
                marker_start = compressed_input.index(marker)
                # decompress what was found before the marker
                decompressed_output += compressed_input[:marker_start]
                # the marker itself is not included in the decompressed output
                compressed_input = compressed_input[marker_start:]
                compressed_input = compressed_input[len(marker):]
                # to decompress the marker, take the subsequent character_count
                # characters and repeat them repeat_count times
                character_count = int(marker[1:-1].split(MARKER_SEPARATOR)[0])
                repeat_count = int(marker[1:-1].split(MARKER_SEPARATOR)[1])
                decompressed_output += compressed_input[:character_count] * repeat_count
                # then, continue reading the file after the repeated data
                compressed_input = compressed_input[character_count:]
            except AttributeError:
                # no marker found, decompress the rest of the input and finish
                # the process
                decompressed_output += compressed_input
                break
        # should be 152851
    ...
    with open(join(getcwd(), SELF_DIR_NAME, INPUT_TXT_NAME), "r") as f:
        ...
        while True:
            try:
                ...
                marker_start = compressed_input.index(marker)
                # decompress what was found before the marker
                decompressed_output_length += len(compressed_input[:marker_start])
                # the marker itself is not included in the decompressed output
                compressed_input = compressed_input[marker_start:]
                compressed_input = compressed_input[len(marker):]
                # to decompress the marker, take the subsequent character_count
                ...
                character_count = int(marker[1:-1].split(MARKER_SEPARATOR)[0])
                repeat_count = int(marker[1:-1].split(MARKER_SEPARATOR)[1])
                to_add = compressed_input[:character_count] * repeat_count
                if marker_pattern.search(to_add) is not None:
                    # there are markers in the decompressed data; markers within
                    # decompressed data are decompressed
                    compressed_input = to_add + compressed_input[character_count:]
                else:
                    # no markers in the decompressed data; add them just to the
                    # output
                    decompressed_output_length += len(to_add)
                    compressed_input = compressed_input[character_count:]
            except AttributeError:
                # no marker found, decompress the rest of the input and finish
                # the process
                decompressed_output_length += len(compressed_input)
                break
        # should be 11797310782
        print(decompressed_output_length)
