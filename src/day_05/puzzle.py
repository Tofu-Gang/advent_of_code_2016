...
def puzzle_01() -> str:
    ...
    character_index = len(HASH_PREFIX)
    return "".join(next(md5_hash)[character_index]
...
def puzzle_02() -> str:
    ...
    password = list("        ")
    md5_hash = md5_hash_generator()
    position_index = len(HASH_PREFIX)
    character_index = position_index + 1

    while " " in password:
        try:
            interesting_hash = next(md5_hash)
            position = int(interesting_hash[position_index])
            character = interesting_hash[character_index]
