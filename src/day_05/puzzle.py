...
def md5_hash_generator() -> Generator[str, None, None]:
    """
    Generates an MD5 hash of the given Door ID and an increasing integer index
    (starting with 0). Only hashes that start with a desired prefix are yielded.
    """

    for index in count():
        md5_hash = md5(bytes(DOOR_ID + str(index), encoding="utf8")).hexdigest()
...
def puzzle_01() -> None:
    ...
    :return: None; Answer should be 1a3099aa.
    """

    md5_hash = md5_hash_generator()
    print_puzzle_solution("".join(next(md5_hash)[5] for _ in range(8)))
...
def puzzle_02() -> None:
    ...
    :return: None; Answer should be 694190cd.
    """
    ...
    while " " in password:
        try:
            interesting_hash = next(md5_hash)
            position = int(interesting_hash[5])
            character = interesting_hash[6]
            ...
            pass
    print_puzzle_solution("".join(password))
