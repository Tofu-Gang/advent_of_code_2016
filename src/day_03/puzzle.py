...
def puzzle_01() -> None:
    ...
    :return: None; Answer should be 917.
    """

    with open(INPUT_FILE_PATH, "r") as f:
        pattern = compile(r"\d+")
        triangles = [tuple(map(lambda element: int(element),
                               pattern.findall(line)))
                     for line in f.readlines()]
        print_puzzle_solution(len(tuple(filter(_is_triangle, triangles))))
...
def puzzle_02() -> None:
    ...
    :return: None; Answer should be 1649.
    """

    with open(INPUT_FILE_PATH, "r") as f:
        pattern = compile(r'\d+')
        lines = [tuple(map(lambda element: int(element),
                           pattern.findall(line)))
                 for line in f.readlines()]
