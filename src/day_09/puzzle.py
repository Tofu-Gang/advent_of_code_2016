...
def puzzle_01() -> None:
    ...
    :return: None; Answer should be 152851.
    """
    ...
    with open(INPUT_FILE_PATH, "r") as f:
        ...
        while True:
            try:
                ...
                decompressed_length += marker_start
                # the marker itself is not included in the decompressed output
                compressed_input = compressed_input[marker_start + len(marker):]
                # to decompress the marker, take the subsequent character_count
                # characters and repeat them repeat_count times
                character_count, repeat_count = tuple(map(
                    lambda value: int(value),
                    compile(r"(\d+)").findall(marker)))
                decompressed_length += character_count * repeat_count
                ...
                # the process
                decompressed_length += len(compressed_input)
                print_puzzle_solution(decompressed_length)
                break
...
def puzzle_02() -> None:
    ...
    :return: None; Answer should be 11797310782.
    """
    ...
    with open(INPUT_FILE_PATH, "r") as f:
        ...
        while True:
            try:
                ...
                compressed_input = compressed_input[marker_end:]
                # to decompress the marker, take the subsequent character_count
                # characters and repeat them repeat_count times
                character_count, repeat_count = tuple(map(
                    lambda value: int(value),
