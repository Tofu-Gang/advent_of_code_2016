...
INSTRUCTIONS = {
    INSTRUCTION_UP: lambda row, column, bound:
    (max(0, row - 1), column),
    INSTRUCTION_DOWN: lambda row, column, bound:
    (min(bound - 1, row + 1), column),
    INSTRUCTION_LEFT: lambda row, column, bound:
    (row, max(0, column - 1)),
    INSTRUCTION_RIGHT: lambda row, column, bound:
...
def puzzle_01() -> None:
    ...
    :return: None; Answer should be 45973.
    """
...
def puzzle_02() -> None:
    ...
    :return: None; Answer should be 27CA4.
    """
    ...
    with open(INPUT_FILE_PATH, "r") as f:
        ...
        for instruction_line in instruction_lines:
            for instruction in instruction_line:
                ...
                if THE_REAL_KEYPAD[new_row][new_column] is not None:
                    row = new_row
                    column = new_column
