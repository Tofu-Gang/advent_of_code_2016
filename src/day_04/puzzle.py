...
class Room(object):
    ...
    def __init__(self, room_line: str):
        ...
        self._name = " ".join(compile(r"([a-z]+)-+").findall(self._room_line))
        self._sector_id = int(compile(r'\d+').findall(self._room_line)[0])
        self._checksum = compile(r'\[([a-z]+)]').findall(self._room_line)[0]
...
def puzzle_01() -> None:
    ...
    :return: None; Answer should be 173787.
    """
...
def puzzle_02() -> None:
    ...
    :return: None; Answer should be 548.
    """

    with open(INPUT_FILE_PATH, "r") as f:
        rooms = [Room(room_line) for room_line in f.readlines()]
        print_puzzle_solution(next(filter(
            lambda room: GOAL_DECRYPTED_ROOM_NAME == room.decrypted_name,
            rooms)).sector_id)
