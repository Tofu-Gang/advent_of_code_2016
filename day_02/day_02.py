    ...
    with open(join(getcwd(), SELF_DIR_NAME, INPUT_TXT_NAME), "r") as f:
        ...
        for line in lines:
            for instruction in line:
                if instruction == INSTRUCTION_UP:
                    row = max(0, row - 1)
                elif instruction == INSTRUCTION_DOWN:
                    row = min(len(KEYPAD) - 1, row + 1)
                elif instruction == INSTRUCTION_LEFT:
                    column = max(0, column - 1)
                elif instruction == INSTRUCTION_RIGHT:
                    column = min(len(KEYPAD) - 1, column + 1)
                else:
                    # shouldn't happen
                    raise Exception()
    ...
    with open(join(getcwd(), SELF_DIR_NAME, INPUT_TXT_NAME), "r") as f:
        ...
        for line in lines:
            for instruction in line:
                if instruction == INSTRUCTION_UP:
                    new_row = max(0, row - 1)
                    if THE_REAL_KEYPAD[new_row][column] is not None:
                        row = new_row
                elif instruction == INSTRUCTION_DOWN:
                    new_row = min(len(THE_REAL_KEYPAD) - 1, row + 1)
                    if THE_REAL_KEYPAD[new_row][column] is not None:
                        row = new_row
                elif instruction == INSTRUCTION_LEFT:
                    new_column = max(0, column - 1)
                    if THE_REAL_KEYPAD[row][new_column] is not None:
                        column = new_column
                elif instruction == INSTRUCTION_RIGHT:
                    new_column = min(len(THE_REAL_KEYPAD) - 1, column + 1)
                    if THE_REAL_KEYPAD[row][new_column] is not None:
                        column = new_column
                else:
                    # shouldn't happen
